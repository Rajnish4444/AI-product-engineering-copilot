...
export const UserStory = z.object({
  as_a: z.string().min(1).describe("The role or persona this story serves"),
  i_want: z.string().min(1).describe("The capability the user is asking for"),
  so_that: z.string().min(1).describe("The outcome or reason the capability matters"),
});
export type UserStory = z.infer<typeof UserStory>;

export const AcceptanceCriterion = z.object({
  given: z.string().min(1).describe("Preconditions or context"),
  when: z.string().min(1).describe("The user action or system event"),
  then: z.string().min(1).describe("The observable outcome"),
});
